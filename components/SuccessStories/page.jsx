...
export default function SuccessStories() {
  ...
  return (
    <section className="w-full flex flex-col items-center justify-center py-20 md:py-32 px-2 md:px-6 bg-black">
      ...
      <div className="w-full max-w-7xl relative flex items-center justify-center pb-12">
        {/* Prev Button */}
        <button
          type="button"
          aria-label="Previous"
          className="hidden md:flex absolute left-4 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-[#181B22] border border-[#23252C] shadow-lg items-center justify-center hover:bg-[#23252C] transition"
          onClick={() => swiperRef.current?.slidePrev()}
        >
          <FaChevronLeft className="text-xl text-[#A6CCFF]" />
        </button>
        {/* Swiper */}
        <Swiper
        ...
        >
          {stories.map((story, idx) => (
            <SwiperSlide key={idx} className="flex items-center justify-center">
              <div className="flex flex-col w-full max-w-[480px] rounded-2xl bg-black border border-[#23252C] shadow-lg p-6 md:p-8 relative mx-auto" style={{boxShadow: '0 2px 12px 0 rgba(63,105,255,0.10), inset 0 8px 32px 0 rgba(255,255,255,0.08)'}}>
        ...
        <button
          type="button"
          aria-label="Next"
          className="hidden md:flex absolute right-4 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-[#181B22] border border-[#23252C] shadow-lg items-center justify-center hover:bg-[#23252C] transition"
          onClick={() => swiperRef.current?.slideNext()}
        >
          <FaChevronRight className="text-xl text-[#A6CCFF]" />
        </button>
