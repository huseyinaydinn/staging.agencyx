  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center w-full pt-20 pb-10">
        <h1 className="text-3xl md:text-4xl font-light text-white text-center mb-12 drop-shadow-lg">
          Launch Your Business on <span className="font-semibold">Autopilot</span>
        </h1>
...
{idx < steps.length - 1 && (
  ...
  />
            ...
            <div className="flex-1 w-full">
              {activeStep === 0 && (
                <div className="flex flex-col justify-center h-full gap-5">
                  <div>
                    <label className="block text-gray-300 text-[15px] mb-2">Business Name</label>
                    <input
                      type="text"
                      name="businessName"
                  ...
                  <div>
                    <label className="block text-gray-300 text-[15px] mb-2">Business Type</label>
                    <input
                      type="text"
                      name="businessType"
                  ...
                  <div>
                    <label className="block text-gray-300 text-[15px] mb-2">Business Size</label>
                    <div className="flex flex-wrap gap-6 mt-1">
                      {['1-10 employees', '11-50 employees', '51-200 employees', '200+ employees'].map((size) => (
                        <label key={size} className="inline-flex items-center text-gray-400 text-[15px] gap-2 cursor-pointer">
                          <input type="radio" name="business-size" value={size} checked={form.businessSize === size} onChange={handleRadio} className="accent-blue-500 w-4 h-4" />
                          <span>{size}</span>
                        </label>
                      ))}
                    </div>
              ...
              {activeStep === 1 && (
                <div className="grid grid-cols-2 gap-5 h-full items-center justify-center">
                  <ServiceCard icon={<FaGlobe className="text-2xl" />} label="Website" checked={form.services.includes('Website')} onChange={handleService} />
                  <ServiceCard icon={<FaChartBar className="text-2xl" />} label="Analytics" checked={form.services.includes('Analytics')} onChange={handleService} />
                  <ServiceCard icon={<FaRobot className="text-2xl" />} label="AI Agent" checked={form.services.includes('AI Agent')} onChange={handleService} />
                  <ServiceCard icon={<FaCubes className="text-2xl" />} label="Inventory" checked={form.services.includes('Inventory')} onChange={handleService} />
                  <ServiceCard icon={<FaDatabase className="text-2xl" />} label="CRM" checked={form.services.includes('CRM')} onChange={handleService} />
                  <ServiceCard icon={<FaEllipsisH className="text-2xl" />} label="Other" checked={form.services.includes('Other')} onChange={handleService} />
                </div>
              )}
              ...
              {activeStep === 2 && (
                <div className="flex flex-col justify-center h-full gap-5">
                  <div>
                    <label className="block text-gray-300 text-[15px] mb-2">Full Name</label>
                    <input
                      type="text"
                      name="fullName"
                      value={form.fullName}
                  ...
                  <div>
                    <label className="block text-gray-300 text-[15px] mb-2">Email</label>
                    <input
                      type="email"
                      name="email"
                      value={form.email}
                  ...
                  <div>
                    <label className="block text-gray-300 text-[15px] mb-2">Phone</label>
                    <input
                      type="tel"
                      name="phone"
                      value={form.phone}
                  ...
                  <div>
                    <label className="block text-gray-300 text-[15px] mb-2">Address</label>
                    <input
                      type="text"
                      name="address"
                      value={form.address}
          ...
          <div className="flex justify-end w-full mt-8 gap-4">
            ...
            >
              {activeStep < 2 ? steps[activeStep + 1].label : 'Submit'}
            </button>
          </div>
        </div>
      </div>
